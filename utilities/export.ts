 ...
 */
export const exportMeetingData = (meeting: Meeting, attendees: Attendee[]): void => {
  ...
  const meetingInfo = [
    ...
    ['Attendance Rate:', attendees.length > 0 
  ...
  for (let i = 0; i < meetingInfo.length; i++) {
    ...
    if (i === 0 || i === 13 || i === 19) {
      ...
      for (let j = 0; j < attendeesHeader.length; j++) {
        ...
        if (ws[headerCell]) {
          ws[headerCell].s = { font: { bold: true }, fill: { fgColor: { rgb: "EFEFEF" } } };
        }
      }
    }
  }
  ...
  // Generate filename
  const filename = `meetcheck_${meeting.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${formatDateForFilename(meeting.date)}_${meeting.id}.xlsx`;
