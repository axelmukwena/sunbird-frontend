...
export interface Column<TRow, TSummaryRow = unknown> {
  ...
  readonly renderEditCell?: Maybe<
    (props: RenderTextEditCellProps<TRow, TSummaryRow>) => ReactNode
  >;
...
export interface RenderTextEditCellProps<TRow, TSummaryRow = unknown> {
  column: CalculatedColumn<TRow, TSummaryRow>;
  row: TRow;
  rowIdx: number;
...
export interface RenderRowProps<TRow, TSummaryRow = unknown>
  ...
  selectedCellEditor: ReactElement<RenderTextEditCellProps<TRow>> | undefined;
  onRowChange: (
    column: CalculatedColumn<TRow, TSummaryRow>,
    rowIdx: number,
    newRow: TRow,
